    ...
    int depthSumInverse(vector<NestedInteger>& nestedList) {
        ...
        while (!nestedList.empty()) {
            ...
            for (auto n : nestedList) {
                if (n.isInteger()) pre += n.getInteger();
                else nl.insert(nl.end(), n.getList().begin(), n.getList().end());
            }
            swap(nestedList, nl);
            res += pre;
        }
...
// Space: O(N), Space complexity is equal to the maximum number of active stack calls during the depth-first search.
class Solution {
public:
